 ...
 * 获取年周表
 */
func WeekTable(timestamp int64, base_table string) string {
	tm := time.Unix(timestamp, 0)
	year := tm.Year()
	weekdays := math.Ceil(float64(tm.YearDay()) / 7)
	return fmt.Sprintf("%v_%v%v", base_table[0:len(base_table)-5], year, weekdays)
}
